...
(() => {
    ...
    const PRIZES_INFO = {
        ball: { name: 'Pelota amarilla', emoji: '⚽' },
        ballgoal: {
            name: 'Pelota con arco', emoji: '⚽',
            svg: '<svg viewBox="0 0 52 52" width="44" height="44">' +
                '<path d="M5 47 L5 14 L47 14 L47 47" fill="none" stroke="#ffffff" stroke-width="3"/>' +
                '<line x1="15" y1="14" x2="15" y2="47" stroke="#ffffff" stroke-width="1.5"/>' +
                '<line x1="26" y1="14" x2="26" y2="47" stroke="#ffffff" stroke-width="1.5"/>' +
                '<line x1="37" y1="14" x2="37" y2="47" stroke="#ffffff" stroke-width="1.5"/>' +
                '<line x1="5" y1="25" x2="47" y2="25" stroke="#ffffff" stroke-width="1.5"/>' +
                '<line x1="5" y1="36" x2="47" y2="36" stroke="#ffffff" stroke-width="1.5"/>' +
                '</svg>',
        },
        seesaw: {
            name: 'Sube y baja', emoji: '🛝',
            svg: '<svg viewBox="0 0 52 52" width="44" height="44">' +
                '<line x1="5" y1="47" x2="48" y2="47" stroke="#5d4037" stroke-width="2.5"/>' +
        ...
        pool: { name: 'Piscina', emoji: '🏊' },
        bicycle: { name: '3 bicicletas', emoji: '🚲' },
    };
    const PRIZE_ORDER = ['ball', 'ballgoal', 'seesaw', 'swing', 'stroller', 'pool', 'bicycle'];
    ...
    function chooseBallVariant(variant) {
        state.selectedPrize = state.selectedPrizeType || 'ball';
    ...
    function placePrizeAt(x, y) {
        if (state.selectedPrize === 'ball' || state.selectedPrize === 'ballgoal') {
            ...
            if (!pr || !pr.owned || pr.placed) { sfxHint(); return; }
            if (state.selectedPrize === 'ball' && state.ballStage === 'ball') {
    ...
    function finishPlacement() {
        ...
        state.selectedPrize = null;
        $('placementBanner').classList.add('hidden');
        $('prizeBoard').classList.add('hidden');
        state.running = true;
    ...
    function startParkOpening() {
        ...
        const seesawPr = state.prizes.find((p) => p.type === 'seesaw');
        const ballPr = state.prizes.find((p) => p.type === 'ball');
        const ballgoalPr = state.prizes.find((p) => p.type === 'ballgoal');
        const strollerPr = state.prizes.find((p) => p.type === 'stroller');
        const poolPr = state.prizes.find((p) => p.type === 'pool');
        const bicyclePr = state.prizes.find((p) => p.type === 'bicycle');
        ...
        if (seesawPr && seesawPr.placed) {
            // sube y baja: dos niños coordinados
            const kid1 = addChild('seesaw', seesawPr.x - 30, seesawPr.y - 15, next(), 1);
            const kid2 = addChild('seesaw', seesawPr.x + 30, seesawPr.y - 15, next(), -1);
            kid1.seesawPartner = kid2;
            kid2.seesawPartner = kid1;
        }
        if (ballPr && ballPr.ballX != null) {
            ...
            const arc = ballPr.variant === 'volley' ? 24 : 5;
            const ay = ballPr.variant === 'volley' ? ballPr.archY - 55 : ballPr.archY - 4;
            state.playBall = { t: 0, arc, ax: ballPr.archX, ay, kids: [kid1, kid2], current: 0, shoot: true };
        }
        if (ballgoalPr && ballgoalPr.ballX != null) {
            // pelota con arco según el arco elegido
            const kid1 = addChild('ball', ballgoalPr.ballX - 50, ballgoalPr.ballY, next());
            const kid2 = addChild('ball', ballgoalPr.ballX + 45, ballgoalPr.ballY, next());
            const arc = ballgoalPr.variant === 'volley' ? 24 : 5;
            const ay = ballgoalPr.variant === 'volley' ? ballgoalPr.archY - 55 : ballgoalPr.archY - 4;
            state.playBall = { t: 0, arc, ax: ballgoalPr.archX, ay, kids: [kid1, kid2], current: 0, shoot: true };
        }
        if (strollerPr && strollerPr.placed) {
            // carritos: un niño por carro, se sube adentro y lo maneja
            const carSeats = [
                { x: strollerPr.x - 40, y: strollerPr.y, ph: 0 },
                { x: strollerPr.x, y: strollerPr.y - 20, ph: 2 },
                { x: strollerPr.x + 40, y: strollerPr.y, ph: 4 },
            ];
            carSeats.forEach((s) => {
        ...
        if (poolPr && poolPr.placed) {
            // piscina: niños nadando
            addChild('pool', poolPr.x - 30, poolPr.y, next());
            addChild('pool', poolPr.x + 30, poolPr.y, next());
        }
        if (bicyclePr && bicyclePr.placed) {
            // bicicletas: niños andando
            const bikeSpots = [
                { x: bicyclePr.x - 50, y: bicyclePr.y },
                { x: bicyclePr.x, y: bicyclePr.y - 25 },
                { x: bicyclePr.x + 50, y: bicyclePr.y },
            ];
            bikeSpots.forEach((s) => {
                const kid = addChild('bicycle', s.x, s.y, next());
                kid.bikePhase = Math.random() * Math.PI * 2;
            });
        }
    ...
    function drawPerson(p) {
        ...
        if (p.driving && p.arrived) {
            ...
            ctx.beginPath();
            ctx.arc(x - 12, y + 18 + bob, 6, 0, Math.PI * 2);
            ctx.arc(x + 12, y + 18 + bob, 6, 0, Math.PI * 2);
            ctx.fill();
            // carrocería
            ctx.fillStyle = '#c62828';
            ctx.beginPath();
            ctx.roundRect ? ctx.roundRect(x - 18, y + 3 + bob, 36, 15, 4) : ctx.rect(x - 18, y + 3 + bob, 36, 15);
            ctx.fill();
            // torso del niño adentro
            ctx.fillStyle = p.shirt;
            ctx.beginPath();
            ctx.ellipse(x, y - 2 + bob, 11, 8, 0, 0, Math.PI * 2);
            ctx.fill();
            // brazos al volante
            ...
            ctx.lineCap = 'round';
            ctx.beginPath();
            ctx.moveTo(x - 11, y - 5 + bob); ctx.lineTo(x - 7, y + 1 + bob);
            ctx.moveTo(x + 11, y - 5 + bob); ctx.lineTo(x + 7, y + 1 + bob);
            ctx.stroke();
            // techo
            ctx.fillStyle = '#e53935';
            ctx.beginPath();
            ctx.roundRect ? ctx.roundRect(x - 14, y - 10 + bob, 28, 9, 3) : ctx.rect(x - 14, y - 10 + bob, 28, 9);
            ctx.fill();
            // ventanas
            ctx.fillStyle = '#b3e5fc';
            ctx.fillRect(x - 12, y - 8 + bob, 10, 6);
            ctx.fillRect(x + 2, y - 8 + bob, 10, 6);
            // cabeza asomando
        ...
        if (p.game === 'bicycle') {
            ...
            if (p.bikeRiding) {
                const wheelSpin = (p.playPhase * 4) % (Math.PI * 2);
                ctx.strokeStyle = '#f57c00';
                ctx.lineWidth = 2.5;
                ctx.beginPath();
                ctx.moveTo(x - 16, y + 14 + bob); ctx.lineTo(x, y - 2 + bob); ctx.lineTo(x + 16, y + 14 + bob);
                ctx.moveTo(x, y - 2 + bob); ctx.lineTo(x, y - 8 + bob);
                ctx.moveTo(x - 10, y - 8 + bob); ctx.lineTo(x + 10, y - 8 + bob);
                ctx.stroke();
                ctx.strokeStyle = '#263238';
                ctx.lineWidth = 2;
                ctx.beginPath();
                ctx.arc(x - 16, y + 14 + bob, 8, 0, Math.PI * 2);
                ctx.stroke();
                ctx.beginPath();
                ctx.arc(x + 16, y + 14 + bob, 8, 0, Math.PI * 2);
                ctx.stroke();
                ...
                ctx.lineWidth = 0.8;
                for (let i = 0; i < 4; i++) {
                    const a1 = wheelSpin + i * Math.PI / 2;
                    ctx.beginPath();
                    ctx.moveTo(x - 16 + Math.cos(a1) * 3, y + 14 + bob + Math.sin(a1) * 3);
                    ctx.lineTo(x - 16 + Math.cos(a1) * 7, y + 14 + bob + Math.sin(a1) * 7);
                    ctx.stroke();
                    ctx.beginPath();
                    ctx.moveTo(x + 16 + Math.cos(a1) * 3, y + 14 + bob + Math.sin(a1) * 3);
                    ctx.lineTo(x + 16 + Math.cos(a1) * 7, y + 14 + bob + Math.sin(a1) * 7);
    ...
    function drawItemGraphic(emoji, x, y, size) {
        if (emoji === '🥛') return drawMilkCup(x, y);
    ...
    function drawPlasticBag(x, y) {
        ...
        ctx.beginPath();
        ctx.moveTo(-10, -12); ctx.lineTo(-8, 8); ctx.lineTo(8, 8); ctx.lineTo(10, -12);
        ctx.closePath();
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.moveTo(-4, -12); ctx.quadraticCurveTo(-6, -18, -2, -16);
        ctx.moveTo(4, -12); ctx.quadraticCurveTo(6, -18, 2, -16);
        ctx.strokeStyle = '#90caf9';
        ctx.lineWidth = 2;
        ctx.stroke();
    ...
    function drawTakeoutBox(x, y) {
        // taper tipo takeout
        ctx.save();
        ctx.translate(x, y);
        ctx.fillStyle = '#f5f5f5';
        ctx.beginPath();
        ctx.roundRect ? ctx.roundRect(-10, -8, 20, 18, 3) : ctx.rect(-10, -8, 20, 18);
        ctx.fill();
        ctx.strokeStyle = '#bdbdbd';
        ctx.lineWidth = 1.5;
        ctx.stroke();
    ...
    function drawPlasticJar(x, y) {
        ...
        ctx.beginPath();
        ctx.roundRect ? ctx.roundRect(-8, -8, 16, 18, 4) : ctx.rect(-8, -8, 16, 18);
        ctx.fill();
        ctx.strokeStyle = '#90caf9';
        ctx.lineWidth = 1.5;
        ctx.stroke();
    ...
    function drawMilkCup(x, y) {
        ...
        ctx.fillStyle = '#f5f5f5';
        ctx.beginPath();
        ctx.moveTo(-10, -10); ctx.lineTo(-8, 10); ctx.lineTo(8, 10); ctx.lineTo(10, -10);
        ctx.closePath();
        ctx.fill();
        ctx.strokeStyle = '#bdbdbd';
        ctx.lineWidth = 1.5;
    ...
    function drawTeaCup(x, y) {
        // vaso descartable de té
        ctx.save();
        ctx.translate(x, y);
        ctx.fillStyle = '#fff3e0';
        ctx.beginPath();
        ctx.moveTo(-9, -10); ctx.lineTo(-7, 10); ctx.lineTo(7, 10); ctx.lineTo(9, -10);
        ctx.closePath();
        ctx.fill();
        ctx.strokeStyle = '#ffcc80';
        ctx.lineWidth = 1.5;
    ...
    function drawJuiceBox(x, y) {
        // cajita de jugo
        ctx.save();
        ctx.translate(x, y);
        ctx.fillStyle = '#fff9c4';
        ctx.beginPath();
        ctx.roundRect ? ctx.roundRect(-8, -10, 16, 20, 3) : ctx.rect(-8, -10, 16, 20);
        ctx.fill();
    ...
    function drawPrizeGraphic(type, x, y, bob, variant) {
        ...
        ctx.translate(x, y);
        ctx.scale(1.7, 1.7);
        if (type === 'swing') ctx.scale(1.25, 1.25); // columpios más grandes
        else if (type === 'seesaw') ctx.scale(1.25, 1.25); // sube y baja más grande
        switch (type) {
            case 'ball_arch': {
            ...
            case 'seesaw': {
                ...
                ctx.fillStyle = '#8d6e63';
                ctx.beginPath();
                ctx.moveTo(-44, 0); ctx.lineTo(44, 0); ctx.lineTo(0, -14);
                ctx.closePath();
                ctx.fill();
            ...
            case 'stroller': {
                ...
                cars.forEach((c) => {
                    ...
                    ctx.fillStyle = '#c62828';
                    ctx.beginPath();
                    ctx.roundRect ? ctx.roundRect(-12, -8, 24, 10, 3) : ctx.rect(-12, -8, 24, 10);
                    ctx.fill();
                    // techo
                    ctx.fillStyle = '#e53935';
                    ctx.beginPath();
                    ctx.roundRect ? ctx.roundRect(-6, -14, 14, 7, 2) : ctx.rect(-6, -14, 14, 7);
                    ctx.fill();
                    // ventanas
                    ctx.fillStyle = '#b3e5fc';
            ...
            case 'pool': {
                // piscina más grande
                const poolW = 100, poolH = 50;
                ctx.fillStyle = '#90caf9';
                ctx.beginPath();
                ctx.roundRect ? ctx.roundRect(-poolW/2, -poolH/2, poolW, poolH, 8) : ctx.rect(-poolW/2, -poolH/2, poolW, poolH);
                ctx.fill();
                ...
                ctx.lineWidth = 2;
                ctx.beginPath();
                for (let i = -poolW/2 + 5; i <= poolW/2 - 5; i += 2) {
                    ctx.lineTo(i, -5 + Math.sin(state.bob * 3 + i * 0.2) * 3);
                }
                ctx.stroke();
                ctx.beginPath();
                for (let i = -poolW/2 + 5; i <= poolW/2 - 5; i += 2) {
                    ctx.lineTo(i, 5 + Math.sin(state.bob * 2.5 + i * 0.25 + 1) * 2);
                }
                ctx.stroke();
                break;
            }
            case 'bicycle': {
                ...
                bikes.forEach((b) => {
                    ...
                    ctx.lineWidth = 2;
                    ctx.beginPath();
                    ctx.moveTo(-10, 4); ctx.lineTo(-2, -6); ctx.lineTo(10, 4);
                    ctx.moveTo(-2, -6); ctx.lineTo(0, -10);
    ...
    function drawPrizes() {
        state.prizes.forEach((pr) => {
            if (pr.type === 'ball' || pr.type === 'ballgoal') {
            ...
            if (!pr.placed) return;
            if (pr.type === 'stroller' && state.people.some((p) => p.driving && p.arrived)) return;
            if (pr.type === 'bicycle' && state.people.some((p) => p.game === 'bicycle' && p.bikeRiding)) return;
    ...
    function update(dt) {
        ...
        state.people.forEach((p) => {
            ...
            if (!p.arrived) {
                ...
                if (p.game === 'swing') {
                    ...
                    if (!p.bikeRiding) {
                        const bikeKids = state.people.filter((q) => q.game === 'bicycle');
                        const allArrived = bikeKids.every((q) => q.arrived);
                        if (allArrived && state.bikeTimer == null) state.bikeTimer = 1;
                        p.ox = p.baseX;
                        p.oy = p.baseY - 14;
                    } else {
                        const a = state.bob * 0.8 + p.bikePhase;
                        p.ox = p.baseX + Math.cos(a) * 120;
                        p.oy = p.baseY + Math.sin(a * 1.3) * 80;
                    }
                }
            } else {
                p.ox = p.x;
        ...
        if (state.bikeTimer != null) {
            state.bikeTimer -= dt;
            if (state.bikeTimer <= 0) {
                state.bikeTimer = null;
                state.people.forEach((p) => { if (p.game === 'bicycle') p.bikeRiding = true; });
            }
        }
    ...
    function bindEvents() {
        ...
        // botones de acción táctiles: patear y agarrar/tirar la pelota
        const bindAction = (id, fn) => {
            const el = $(id);
            el.addEventListener('pointerdown', (e) => { e.preventDefault(); fn(); });
        };
        bindAction('btnKick', () => kickBall());
        bindAction('btnGrab', () => toggleHoldBall());

        // mostrar controles táctiles solo si el dispositivo soporta touch
